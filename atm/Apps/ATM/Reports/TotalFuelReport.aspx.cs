...
public partial class Apps_ATM_Reports_TotalFuelReport : ATMReportPage
{
    ...
    protected void btnGenerate_Click(object sender, EventArgs e)
    {
        try
        {
            for (var i = 0; i < lbSygmaCenterNo.Items.Count; i++)
            {
                ...
                {
                    centerNumber = lbSygmaCenterNo.Items[i + 1].Value;
                    GetSelectedCenters(centerNumber);
                }
                else if (lbSygmaCenterNo.Items[i].Selected)
                {

                    centerNumber = lbSygmaCenterNo.Items[i].Value;
                    GetSelectedCenters(centerNumber);

                }
            }

            var reportHasRecords = BuildExcelReport("2", Convert.ToDateTime(txtStartDate.Text),
                Convert.ToDateTime(txtEndDate.Text), SygmaCenterNo); // 1 – Report Id, Startdate, Enddate
            if (!reportHasRecords)
            {
